...
async function syncAppointments(appointments: Appointment[], prev: Appointment[], onError: (msg: string) => void) {
  ...
  for (const a of appointments) {
    ...
    if (error) { console.error('[syncAppointments] upsert error:', error); onError('Sync failed â data may not be saved. Check connection.'); }
  }
}
...
async function syncSalonServices(services: SalonService[], prev: SalonService[], onError: (msg: string) => void) {
  ...
  for (const s of deleted) {
    ...
    if (error) { console.error('[syncSalonServices] delete error:', error); onError('Sync failed â data may not be saved. Check connection.'); }
  }
  ...
  for (const s of services) {
    ...
    if (error) { console.error('[syncSalonServices] upsert error for', s.id, error); onError('Sync failed â data may not be saved. Check connection.'); }
  }
}
...
async function syncTurnCriteria(criteria: TurnCriteria[], onError: (msg: string) => void) {
  for (const c of criteria) {
    ...
    if (error) { console.error('[syncTurnCriteria] error:', error); onError('Sync failed â data may not be saved. Check connection.'); }
  }
}
...
async function syncCalendarDays(days: CalendarDay[], prev: CalendarDay[], onError: (msg: string) => void) {
  ...
  for (const d of deleted) {
    ...
    if (error) { console.error('[syncCalendarDays] delete error:', error); onError('Sync failed â data may not be saved. Check connection.'); }
  }
  ...
  for (const d of days) {
    ...
    if (error) { console.error('[syncCalendarDays] upsert error:', error); onError('Sync failed â data may not be saved. Check connection.'); }
  }
}
