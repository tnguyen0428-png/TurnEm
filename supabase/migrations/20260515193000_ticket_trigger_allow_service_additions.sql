-- Allow the queue→ticket trigger to add services when the queue entry's
-- services array changes mid-visit (e.g. receptionist edits a client to
-- tack on an Eyebrow Wax while she's getting a Gel Manicure).
--
-- The previous version bailed out via a `source_row_id ANY(auto_attributed_sources)`
-- guard the moment it had processed the row once. That guard was meant to
-- prevent redundant work, but it also blocks legitimate UPDATEs where the
-- services array grew. The per-service `same-name-staff` guard further
-- down already prevents duplicate inserts, so it's safe to re-run the
-- service loop on every fire.
--
-- Everything else (the closed/voided ticket short-circuit, the ticket
-- INSERT path, the same-name-staff guard, totals recompute, source-id
-- attribution) is preserved verbatim from the prior definition.
...
BEGIN
  ...
  FROM tickets
  WHERE queue_entry_id = p_visit_id
  LIMIT 1;
  ...
  -- NOTE: re-fire guard removed. We now re-run the service loop on every
  -- UPDATE so mid-visit service additions land on the ticket. The
  -- per-service same-name-staff guard below de-dupes existing lines.
  ...
  IF p_services IS NOT NULL THEN
    FOREACH v_svc_name IN ARRAY p_services LOOP
      ...
      -- Same-name-staff guard: if a line with this service+staff already
      -- exists (any qe, including NULL), don't add another.
      IF EXISTS (
  ...
  -- Mark source row as attributed even if every line was guarded out.
